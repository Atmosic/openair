...
#define CMD_NAME          "BLESCANRSPDATA"
#define CMD_PARM_FMT      "B(0~1),A(~" AT_CMD_MAX_ADV_DATA_LEN_STR ")"
#define CMD_PARM_DESC     "<act_idx>,<scan response>"
...
LOG_MODULE_REGISTER(blescanrspdata, CONFIG_AT_CMD_SET_LOG_LEVEL);
...
static void fn_cmd_handler(at_cmd_param_t *param)
{
	...
	if (param->type == at_cmd_type_exec) {
		...
		if (arr_len > AT_CMD_MAX_ADV_DATA_LEN) {
			LOG_ERR("Scan response data too long: %u, max %u", arr_len,
				AT_CMD_MAX_ADV_DATA_LEN);
			AT_CMD_APP_ERR_TO_PARAM(AT_ERR_ADV_SCAN_DATA_TOO_LONG, param);
			return;
		}

		memcpy(ctx->scan_data[idx], arr, arr_len);
		ctx->scan_data_len[idx] = arr_len;
		LOG_DBG("Set scan response data idx=%u, len=%u", idx, arr_len);
		if (ctx->adv_enabled[idx]) {
			at_cmd_result_t result = at_cmd_adv_update_data(ctx, idx);
			AT_CMD_RESULT_TO_PARAM(result, param);
			if (param->err) {
				LOG_ERR("Failed to update scan response data on instance %u", idx);
				return;
			}
			LOG_DBG("update scan response data on instance %u", idx);
		} else {
			LOG_DBG("Advertising not enabled on instance %u, scan response data will be"
				" applied when advertising is started",
				idx);
		}
		...
		for (uint8_t i = 0; i < AT_CMD_ADV_MAX_INST; i++) {
			AT_CMD_RSP_REPEAT(param->ch, param->cmd, i, AT_CMD_ADV_MAX_INST, 2, i,
					  ctx->scan_data[i], ctx->scan_data_len[i]);
		}
	}
}

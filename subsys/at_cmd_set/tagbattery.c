...
static void fn_cmd_handler(at_cmd_param_t *param)
{
	...
	if (!ctx->callbacks.tag_cb.get_battery_cb) {
		param->err = AT_CMD_ERR_NOT_SUPPORT;
		return;
	}

	uint8_t percentage = 0;
	at_cmd_tag_err_t ret = ctx->callbacks.tag_cb.get_battery_cb(&percentage);
	...
	at_cmd_resp(param->ch, at_all, param->cmd, 0, RSP_PARM_NUM, percentage);
}

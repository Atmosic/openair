/*
 * Copyright (c) 2026 Atmosic
 *
 * SPDX-License-Identifier: LicenseRef-Atmosic
 */

#include <zephyr/logging/log.h>
#include "at_cmd.h"
#include "at_cmd_set.h"

#define CMD_NAME      "TAGMODE"
#define CMD_PARM_FMT  "B(1~7)"
#define CMD_PARM_DESC "<0x01: fmna|0x02: fhn|0x04: stf|0x07: all>"
#define CMD_PARM_NUM  1
#define RSP_PARM_FMT  "B"
#define RSP_PARM_NUM  1

LOG_MODULE_REGISTER(tagmode, CONFIG_AT_CMD_SET_LOG_LEVEL);

static void fn_cmd_handler(at_cmd_param_t *param)
{
	at_cmd_ctx_t *ctx = at_cmd_ctx_get();
	uint8_t mode = 0;

	LOG_DBG("at_type (%d), err (%d)", param->type, param->err);

	if (param->err != AT_CMD_ERR_NO_ERROR) {
		return;
	}

	if (param->type == at_cmd_type_exec) {
		if (!ctx->callbacks.tag_cb.mode_cb) {
			param->err = AT_CMD_ERR_NOT_SUPPORT;
			return;
		}

		mode = AT_PASR_GET_PARAM(param, u8, 0);
		if (!mode) {
			param->err = AT_CMD_ERR_WRONG_ARGU_CONTENT;
			return;
		}

		at_cmd_tag_err_t ret = ctx->callbacks.tag_cb.mode_cb(AT_CMD_TAG_MODE_OP_SET, &mode);
		if (ret != AT_CMD_TAG_NO_ERR) {
			param->err = AT_CMD_ERR_SPECIFIC_ERR;
			param->app_err = ret;
		}
		return;
	}

	if (param->type != at_cmd_type_query) {
		param->err = AT_CMD_ERR_WRONG_EXECUTE_TYPE;
		return;
	}

	if (!ctx->callbacks.tag_cb.mode_cb) {
		param->err = AT_CMD_ERR_NOT_SUPPORT;
		return;
	}

	at_cmd_tag_err_t ret = ctx->callbacks.tag_cb.mode_cb(AT_CMD_TAG_MODE_OP_GET, &mode);
	if (ret != AT_CMD_TAG_NO_ERR) {
		param->err = AT_CMD_ERR_SPECIFIC_ERR;
		param->app_err = ret;
		return;
	}

	at_cmd_resp(param->ch, at_all, param->cmd, 0, RSP_PARM_NUM, mode);
}

AT_COMMAND(CMD_NAME, CMD_PARM_FMT, CMD_PARM_NUM, fn_cmd_handler, CMD_PARM_DESC, RSP_PARM_FMT,
	   RSP_PARM_NUM);

...
#define CMD_NAME      "BLEGAPGETCONNSTAT"
#define CMD_PARM_FMT  "B(0~" STRINGIFY(CONN_IDX_MAX) ")"
#define CMD_PARM_DESC "<conn_idx>"
#define CMD_PARM_NUM  1
#define RSP_PARM_FMT  "B,B,A(6~6),W,W,W"
#define RSP_PARM_NUM  6

LOG_MODULE_REGISTER(blegapgetconnstat, CONFIG_AT_CMD_SET_LOG_LEVEL);
...
static void fn_cmd_handler(at_cmd_param_t *param)
{
	...
	at_cmd_resp(param->ch, at_all, param->cmd, 0, RSP_PARM_NUM, (uint8_t)info.role,
		    (uint8_t)info.le.dst->type, peer_addr, (uint16_t)BT_ADDR_SIZE,
		    BT_GAP_US_TO_CONN_INTERVAL(info.le.interval_us), info.le.latency,
		    info.le.timeout);
}

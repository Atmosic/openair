...
LOG_MODULE_REGISTER(tagfmnamfi, CONFIG_AT_CMD_SET_LOG_LEVEL);
...
static void fn_cmd_handler(at_cmd_param_t *param)
{
	...
	if (!ctx->callbacks.tag_cb.fmna_mfi_log_cb) {
		param->err = AT_CMD_ERR_NOT_SUPPORT;
		return;
	}

	at_cmd_tag_err_t ret = ctx->callbacks.tag_cb.fmna_mfi_log_cb();

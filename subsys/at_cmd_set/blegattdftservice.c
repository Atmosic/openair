...
#define CMD_NAME      "BLEGATTDFTSERVICE"
...
LOG_MODULE_REGISTER(blegattdftservice, CONFIG_AT_CMD_SET_LOG_LEVEL);
...
static void fn_cmd_handler(at_cmd_param_t *param)
{
	...
	if (param->type == at_cmd_type_exec) {
		...
		/* Parse UUID (BT_UUID_SIZE_128 bytes, MSB-first) */
		...
		at_cmd_result_t result = at_cmd_gatt_dft_service_set(sec_prop, uuid);
		...
		at_cmd_result_t result = at_cmd_gatt_dft_service_get(&sec_prop, uuid);
		if (result.err || result.app_err) {
			AT_CMD_RESULT_TO_PARAM(result, param);
			return;
		}

		/* Return sec_prop as MSB-first 2 bytes, UUID as BT_UUID_SIZE_128 bytes */

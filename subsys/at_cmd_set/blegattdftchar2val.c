...
#define CMD_NAME      "BLEGATTDFTCHAR2VAL"
...
LOG_MODULE_REGISTER(blegattdftchar2val, CONFIG_AT_CMD_SET_LOG_LEVEL);
...
static void fn_cmd_handler(at_cmd_param_t *param)
{
	...
	if (param->type == at_cmd_type_exec) {
		...
		at_cmd_result_t result = at_cmd_gatt_dft_char2_val_set(data, len);
		...
		at_cmd_result_t result = at_cmd_gatt_dft_char2_val_get(data, &len);
		if (result.err || result.app_err) {
			AT_CMD_RESULT_TO_PARAM(result, param);
			return;
		}

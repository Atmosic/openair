...
static bool ras_transit_evt_work_put(ras_trans_work_evt_t evt, bool is_on_demand,
				     uint16_t ranging_cnt);
...
static void ras_timeout_work_handler(struct k_work *work)
{
	...
	LOG_DBG("%s:%d", __func__, info->event);
...
static void ras_timeout_evt_work_put(ras_timeout_work_evt_t evt, uint16_t rd_cnt, k_timeout_t delay)
{
	...
	if (ras.ras_timeout_work.pending_cnt > 1) {
		LOG_ERR("event overwrite:%d->%d", ras.ras_timeout_work.event, evt);
	}
...
static ssize_t ras_realtime_rd_ccc_write_cfm(struct bt_conn *conn, const struct bt_gatt_attr *attr,
					     uint16_t value)
{
	LOG_INF("%s: ccc:%u", __func__, value);
...
static int ras_cp_cmp_rd_rsp(struct bt_conn *conn, uint16_t ranging_counter)
{
	ras.ras_cp_rsp.opcode = RAS_CP_RSP_OPCODE_COMPLETE_RD_RSP;
	ras.ras_cp_rsp.para.ranging_cnt = ranging_counter;
	uint16_t len = sizeof(ras.ras_cp_rsp.para.ranging_cnt) + RAS_CP_OPCODE_LEN;
	int err = ras_cp_indicate(conn, len);
	if (!err) {
		// Wait Ack within RAS_CP_ACK_DATA_TIMEOUT
		ras_timeout_evt_work_put(RAS_TIMEOUT_WORK_EVT_WAIT_ACK, ranging_counter,
					 RAS_CP_ACK_DATA_TIMEOUT);
	}
	return err;
}

static int ras_cp_rsp_code_rsp(struct bt_conn *conn, uint8_t rsp_code)
{
	ras.ras_cp_rsp.opcode = RAS_CP_RSP_OPCODE_RSP_CODE;
	ras.ras_cp_rsp.para.code_val = rsp_code;
	uint16_t len = sizeof(ras.ras_cp_rsp.para.code_val) + RAS_CP_OPCODE_LEN;
	return ras_cp_indicate(conn, len);
}
				       ...
				       uint8_t flags)
{
	...
	case RAS_CP_CMD_OPCODE_GET_RD: {
		if (len <= sizeof(cp_cmd->para.ranging_cnt)) {
			ras_cp_rsp_code_rsp(conn, RAS_CP_RSP_INVALID_PARAMETER);
			break;
		}
		if (!ras_ind_notify_is_enable(conn, BT_UUID_RAS_ONDEMAND_RD)) {
			ras_cp_rsp_code_rsp(conn, RAS_CP_RSP_OPCODE_NOT_SUPPORTED);
			break;
		}
		if (!ras_transit_evt_work_put(RAS_TRANS_WORK_EVT_START, true,
					      cp_cmd->para.ranging_cnt)) {
			ras_cp_rsp_code_rsp(conn, RAS_CP_RSP_NO_RECORDS_FOUND);
			break;
		}
	} break;
	case RAS_CP_CMD_OPCODE_ACK_RD: {
		...
		ras_cp_rsp_code_rsp(conn, RAS_CP_RSP_SUCCESS);
	...
	case RAS_CP_CMD_PCODE_ABORT_OP:
...
BT_GATT_SERVICE_DEFINE(
	...
	BT_GATT_CCC_MANAGED(((struct _bt_gatt_ccc[]){
				    BT_GATT_CCC_INITIALIZER(ras_realtime_rd_ccc_changed,
							    ras_realtime_rd_ccc_write_cfm, NULL)}),
			    BT_GATT_PERM_READ_ENCRYPT | BT_GATT_PERM_WRITE_ENCRYPT),
#endif
	BT_GATT_CHARACTERISTIC(BT_UUID_RAS_ONDEMAND_RD, BT_GATT_CHRC_INDICATE | BT_GATT_CHRC_NOTIFY,
			       BT_GATT_PERM_NONE, NULL, NULL, NULL),
	BT_GATT_CCC_MANAGED(((struct _bt_gatt_ccc[]){
				    BT_GATT_CCC_INITIALIZER(ras_ondemand_rd_ccc_changed,
							    ras_ondemand_rd_ccc_write_cfm, NULL)}),
			    BT_GATT_PERM_READ_ENCRYPT | BT_GATT_PERM_WRITE_ENCRYPT),
...
static int ras_cp_indicate(struct bt_conn *conn, uint16_t len)
{
	...
	if (bt_gatt_is_subscribed(conn, attr, BT_GATT_CCC_INDICATE)) {
		...
		ras.ras_cp_para.data = &ras.ras_cp_rsp, ras.ras_cp_para.len = len;
				     ...
				     uint16_t ranging_cnt)
{
	...
	if (ras.ras_trs_work.pending_cnt > 1) {
		LOG_ERR("event overwrite:%d->%d", ras.ras_trs_work.event, evt);
		return false;
	}
...
{
	k_work_cancel(&ras.ras_trs_work.work);
...
static void ras_rd_seg_trans_work_handler(struct k_work *work)
{
	...
	case RAS_TRANS_WORK_EVT_END: {
		if (info->is_on_demand) {
			ras_cp_cmp_rd_rsp(ras.curr_conn, info->ranging_cnt);
		} else {
			ras_rdbuf_free_buf(info->ranging_cnt);
		}
	} break;
	...
	// Trigger next step
	if (info->event != RAS_TRANS_WORK_EVT_END) {
...
static void ras_conn_evt_work_put(ras_conn_work_evt_t evt)
{
	...
	if (ras.ras_conn_work.pending_cnt > 1) {
		LOG_ERR("event overwrite:%d->%d", ras.ras_conn_work.event, evt);
	}
...
static void ras_clear_work(void)
{
	...
	LOG_INF("RAS sts work:%u", ras.sts_work_pending);
}
...
static void ras_cs_procedure_enabled_cb(struct bt_conn *conn,
...
BT_CONN_CB_DEFINE(ras_cb) = {
	...
	.le_cs_procedure_enabled = ras_cs_procedure_enabled_cb,

...
V1_BANNER_REGEX = r"AT_CMD_SET APP_VER=1\.0\.0"
V2_BANNER_REGEX = r"AT_CMD_SET APP_VER=2\.0\.0"
READY_REGEX = r"Ready for AT commands"


def _wait_for(dut: DeviceAdapter, regex: str, timeout: float) -> list[str]:
    lines = dut.readlines_until(regex=regex, timeout=timeout)
    assert any(
        re.search(regex, line) for line in lines

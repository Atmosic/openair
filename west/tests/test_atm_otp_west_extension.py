...
def test_atmotp_list_otp(atmotp_list_otp_args, atmotp_base_subcmd_args):
    ...
    with pytest.raises(Exception):
        subprocess.run(atmotp_list_otp_args, check=True, capture_output=True)
...
def test_atmotp_dump(atmotp_dump_args, atmotp_base_subcmd_args):
    ...
    # Verify that the command fails without the proper flags
    with pytest.raises(Exception):
        subprocess.run(atmotp_dump_args, check=True, capture_output=True)
...
def test_atmotp_get(atmotp_get_args, atmotp_base_subcmd_args):
    ...
    # Verify that the command fails without the proper flags
    with pytest.raises(Exception):
        subprocess.run(atmotp_get_args, check=True, capture_output=True)
    subproc_cmd = atmotp_get_args + atmotp_base_subcmd_args
    call = subprocess.run(
        subproc_cmd + ["--otp", "RRAM_WRITE_LOCK"], check=True, capture_output=True
    )
    bin_output = call.stdout.split(b"\n")[1].decode(
        encoding="utf-8", errors="strict"
    )  # strip OpenOCD print
    expected_len = 7
    assert re.search(rf"0b[0-1]{{{expected_len}}}", bin_output)
    call = subprocess.run(
        subproc_cmd + ["--otp", "53"], check=True, capture_output=True
    )
    ...
    expected_len = 1
    assert re.search(rf"0b[0-1]{{{expected_len}}}", bin_output_53)
    call = subprocess.run(
        subproc_cmd + ["--otp", "RRAM_WRITE_LOCK.5"], check=True, capture_output=True
    )
    bin_output_RRAM_WRITE_LOCK_5 = call.stdout.split(b"\n")[1].decode(
        encoding="utf-8", errors="strict"
    )  # strip OpenOCD print
    expected_len = 1
    assert re.search(rf"0b[0-1]{{{expected_len}}}", bin_output_RRAM_WRITE_LOCK_5)
    call = subprocess.run(
        subproc_cmd + ["--otp", "RRAM_WRITE_LOCK.SECURE_COUNTERS"],
        check=True,
        capture_output=True,
    )
    ...
    expected_len = 1
    assert re.search(
        rf"0b[0-1]{{{expected_len}}}", bin_output_RRAM_WRITE_LOCK_SECURE_COUNTERS
    )
    assert (
        bin_output_53
        == bin_output_RRAM_WRITE_LOCK_5

...
def unescaped_str(arg_str):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(
            "Non-ASCII characters detected in --data parameter. "
...
class SecJrnlCommand(WestCommand):
    ...
    def do_add_parser(self, parser_adder):
        ...
        ratchet_jrnl_parser.add_argument(
            "-y",
            default=False,
            required=False,
            action="store_true",
            help="Skip user input and ratchet Secure Journal",
        )
        create_parser = subparsers.add_parser(
            "create", help="Build a local secure journal binary file"
        )
        create_parser.add_argument(
            "--len",
            type=auto_int,
            required=False,
            help="Length of outputed bin file",
            default=1776,
        )
        create_parser.add_argument(
            "-t",
        ...
        create_parser.add_argument(
            "-o", "--outfile", required=False, type=argparse.FileType("wb"), default="-"
        )
    ...
    def _get_ratchet(self):
        self.openocd.reset_target()
        cmd_ret, _, stderr = self.openocd.execute_cmd(["atm_sec_jrnl_get_ratchet"])
        if cmd_ret != 0:
            print(f"{stderr}")
        # TODO: handle return string output better.
        return auto_int(stderr.split()[-1])
    ...
    def ratchet_jrnl(self, args):
        ...
        if ratchet_val == new_ratchet_val:
            raise RuntimeError(
                "No unratcheted tags found. Cannot ratchet empty data."
        ...
        if not args.y:
            ...
            for tag in iter(sec_jrnl):
                if tag.idx >= ratchet_val:
                    print(
                        f"\t{tag}{wrap_color('   ; Note: this tag is LOCKED and cannot be updated if you ratchet', TermColors.RED) if tag.status.locked else ''}"
                    )
            print(
                f"Are you sure you want to increment the ratchet index from {ratchet_val} to {new_ratchet_val}?"
            )
            answer = input()
            if answer.lower() == "no":
                return
        ...
        self.openocd.reset_target()
        cmd_ret, _, stderr = self.openocd.execute_cmd(
            [f"atm_sec_jrnl_incr_ratchet_to {new_ratchet_val}"]
        )
        if cmd_ret != 0:
            print(f"{stderr}")

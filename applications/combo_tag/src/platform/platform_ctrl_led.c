...
static void led_pattern_process(struct led_blink_param *param, struct k_work *work)
{
	...
	atm_work_reschedule_for_app_work_q(timer_work, K_MSEC(param->pattern[param->pattern_idx].duration_ms));
...
static void led_blink_blocking(struct led_blink_pattern *pattern, uint32_t pattern_num, uint32_t count)
{
	if (!device_is_ready(led_dev)) {
		return;
	}

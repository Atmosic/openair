...
/**
 * @brief Process data through the AES engine
 *
 * @note This function provides synchronize access to the ECB encrypt and
 *       decrypt functions of the AES engine.
 *       Only use the native APIs in non-threaded bare metal.
 *       Only single block can be processed here.
 *
 * @param[in] dest Pointer to write the data after processing
 * @param[in] src Pointer to the data to be processed
 * @param[in] params aes parameters
 * @return atm_aes_res_t result
 */

 ...
 * @file pinmux_34xx.h
 *
 * @brief Pinmux driver interface for 34xx silicon
 *
...
/**
 * @defgroup PINMUX_34XX PINMUX_34XX
 * @ingroup PINMUX
 * @brief Pinmux driver interface for 34xx silicon
 * @{
 */
...
#define GPIO_GET_IRQ(inst) GPIO_##inst##_IRQ
...
#define PIN_READ_GPIO_DATA(pin) \
    GPIO_READ_DATA(PIN2GPIO(pin)) \
...
#define PINMUX_QSPI_SET() do { \
    ...
    PIN_PULL_CLR(PIN_QSPI_CLK); \
    PIN_PULL_CLR(PIN_QSPI_CSN); \
    PIN_PULL_CLR(PIN_QSPI_D0); \
    PIN_PULL_CLR(PIN_QSPI_D1); \
...
#define _PINMUX_GET_PWM_CHANNEL(p) PWM_PIN##p##_CH
#define PINMUX_GET_PWM_CHANNEL(p) _PINMUX_GET_PWM_CHANNEL(p)

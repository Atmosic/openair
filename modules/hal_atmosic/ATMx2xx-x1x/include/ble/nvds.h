 ...
 *                           Out: Number of bytes copied into buf; set to 0 if
 *                           TAG was not found or tried to read tag data
 *                           beyond its size (get_offset >= the length of the
 *                           TAG data), or if buf is too small.  Exception: if
 *                           buf is too small but one of get_offset or get_size
 *                           is non-0, lengthPtr is left intact eventhough no
 *                           data is copied into buf.
 * @param[out] buf           A pointer to the buffer allocated by the caller
 *                           to be filled with the DATA part of the TAG
 *
 * @return  NVDS_OK                  The read operation was performed (TAG was
 *                                   found and buf was large enough)
 ...
 *         NVDS_FAIL       If TAG is not a workaround tag
 *         NVDS_TAG_NOT_DEFINED   Last TAG reached, header filled with garbage
 *         NVDS_CORRUPT           Current TAG is overcoming the NVDS size limit
 ****************************************************************************************
 */

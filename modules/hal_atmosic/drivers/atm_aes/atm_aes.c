...
#include "atm_utils_reg.h"
    ...
    (!defined(CONFIG_SOC_FAMILY_ATM) || defined(CONFIG_ATM_KEY_SIDELOAD))
#define SUPPORT_KEY_SIDE_LOAD
    ...
    size_t num_bytes)
{
...
#endif
	if (byte_reverse) {
	    // If bytes are reversed, partial data must be right justified
	    atm_reg_write_multiple_r(&(CMSDK_AES->DATA_IN_0), BLOCK_WORDS, src,
		remaining_bytes);
	    size_t zero_words =
		(ATM_AES_BLOCK_LEN_BYTES - remaining_bytes) / sizeof(uint32_t);
	...
	if (byte_reverse) {
	    // If bytes are reversed, partial data must be right justified
	    atm_reg_read_multiple_r(&(CMSDK_AES->DATA_OUT_0), BLOCK_WORDS, dest,
		remaining_bytes);
	} else {
	    atm_reg_read_multiple(&(CMSDK_AES->DATA_OUT_0), dest,
		remaining_bytes);

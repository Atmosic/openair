...
/**
 * @brief Check if MAC is in low latency mode
 *
 * This function is used to check the current low latency state.
 *
 * @return       true if in low latency mode, false otherwise
 */
bool atm_mac_is_low_latency_mode(void);

/**
 * @brief Enable or disable low latency mode
 *
 * This function sets the MAC low latency mode.
 *
 * @param[in]    enable    true to enable low latency mode, false to disable
 */
void atm_mac_enable_low_latency_mode(bool enable);

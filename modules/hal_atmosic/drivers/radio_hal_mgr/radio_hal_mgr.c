...
#define MGR_DEFER_IRQn SW_6_IRQn
#endif
...
static int atm_mac_mgr_sys_init(void)
{
    ...
    // Initialize interrupts for management
...
#endif
    NVIC_DisableIRQ(MGR_DEFER_IRQn);
    NVIC_ClearPendingIRQ(MGR_DEFER_IRQn);
#if defined(CONFIG_SOC_FAMILY_ATM) && !defined(CONFIG_BOARD_UNIT_TESTING)
    IRQ_DIRECT_CONNECT(MGR_DEFER_IRQn, IRQ_PRI_UI, SW_6_Handler, 0);
    irq_enable(MGR_DEFER_IRQn);
#else
    NVIC_SetPriority(MGR_DEFER_IRQn, IRQ_PRI_UI);
    NVIC_EnableIRQ(MGR_DEFER_IRQn);
#endif
    return 0;
}
...
#if defined(CONFIG_SOC_FAMILY_ATM) && !defined(CONFIG_BOARD_UNIT_TESTING)
ISR_DIRECT_DECLARE(SW_6_Handler)
#else
void SW_6_Handler(void)
#endif
{
    __UNUSED bool reschedule = atm_mac_mgr_defer_handler();
#if defined(CONFIG_SOC_FAMILY_ATM) && !defined(CONFIG_BOARD_UNIT_TESTING)
    return reschedule;
#endif
}

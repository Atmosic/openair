...
typedef struct {
     ...
     * all information needed to start a given operation when the operation is
     * initially scheduled.
     */
...
/**
 * @brief Start deferred protocol operation
 *
 * Trigger the protocol to start using the radio. The protocol must store all
 * information needed to start a given operation when the operation is initially
 * scheduled. This start function may be used instead of the full API, as long
 * as all operations are scheduled with the critical bit set in their priority.
 *
 * @return True if the kernel should reschedule when the handler runs
 */
typedef bool (*atm_mac_mgr_deferred_start_handler_t)(void);
...
#define PRIORITY_CRITICAL_READ(priority) (((priority) >> 28) & 0b1U)
#define PRIORITY_CRITICAL_SET(priority) (priority) = (priority) | (0b1U << 28)
 ...
 * around each other.
 *
 * @param[in] first The priority of the first operation. This operation
...
/**
 * @brief Register deferred API
 *
 * The deferred variant of the API will start operations in a lower interrupt
 * context, allowing kernel functions to be called. In order to use this
 * option, all manager operations must be called with the critical bit set
 *
 * @param[in] interface Manager interface
 * @param[in] protocol Protocol to register API to
 * @param[in] start function to call to start protocol
 */
void atm_mac_mgr_register_deferred_api(atm_mac_mgr_interface_t interface,
    atm_mac_mgr_protocol_t protocol,
    atm_mac_mgr_deferred_start_handler_t start);
...
/**
 * @brief Trigger interrupt to handle deferred start
 *
 * @note This function may be called from interrupt context.
 */
 ...
 *
 * @return True if the kernel should reschedule
 */
bool atm_mac_mgr_defer_handler(void);

...
enum pc_ctr_sleep_e {
    /**
     * Work is ready to be done immediately.
     * Exit power management critical section as quickly as possible.
     */
    PC_CTR_ACTIVE = 0,

    /**
     * No immediate work, but interrupt is expected soon, and it needs
     * to be handled with very low latency (single digit microseconds).
     */
    PC_CTR_CPU_REST,

    /**
     * No immediate work, but interrupt is expected soon, and it needs
     * to be handled with low latency (double digit microseconds).
     */
    PC_CTR_CPU_SLEEP,

    /**
     * No immediate work and ATLC power is being managed.  Minimum latency
     * to return to active state on the order of triple digit microseconds
     * (see PC_CTR_WRM_LPC).
     */
    PC_CTR_ATLC_WAKING,

...
/**
 * @defgroup NV_MEM_PROT RRAM Protection
 * @ingroup DRIVERS
 * @brief RRAM Protection Driver
 * @{
 */
...
/// RRAM protection granularity
#define RRAM_PROT_BLOCK_SIZE 2048
...
#define RRAM_PROT_SIZE_RNDUP(size)                                                                 \
	((((size) + RRAM_PROT_BLOCK_SIZE - 1) / RRAM_PROT_BLOCK_SIZE) * RRAM_PROT_BLOCK_SIZE)

/**
 * @brief Enable writes to a RRAM region.
 *
 * @note The RRAM region that is enabled may cover one or more
 * RRAM_PROT_BLOCK_SIZE blocks that spans the region.
 *
 ...
 *
 * @note Must be block aligned to RRAM_PROT_BLOCK_SIZE
 *
 * @param[in] offset offset of region from the start of RRAM
 * @param[in] length length of region
 * @return true on success
...
#else
static inline bool rram_write_section_allowed(void)
{
	return true;
}
#endif

#if defined(WRPRPINS_RRAM_REMAP0__BLK0__MASK) || defined(CFG_NO_RRAM_WRITE_CRIT_SECTION)
#define ENTER_RRAM_WRITE_SECTION()                                                                 \
	do {                                                                                       \
	} while (0)
#define LEAVE_RRAM_WRITE_SECTION()                                                                 \
	do {                                                                                       \
	} while (0)
#else
#define ENTER_RRAM_WRITE_SECTION() ATM_BP_CLOCK_ENTER_CRITICAL_SECTION(RRAM_WRITE_MAX_BP_CLOCK)

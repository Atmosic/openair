...
static K_KERNEL_STACK_DEFINE(ble_thread_stack, CONFIG_ATM_BLE_STACK_SIZE);
...
static int ble_driver_init(struct device const *dev)
{
    ...
    // Adjust timing for Zephyr idle critical section
    ATLC_LC_LP_CTRL4__WPSEQ__MODIFY(CMSDK_ATLC_NONSECURE->LC_LP_CTRL4,
	ATLC_LC_LP_CTRL4__WPSEQ__READ(CMSDK_ATLC_NONSECURE->LC_LP_CTRL4) + 3);
...
static int ble_driver_init(struct device const *dev)
{
	...
	ATLC_LC_LP_CTRL0__OSC_SLP_EN__MASK;
    CMSDK_ATLC_NONSECURE->LC_LP_CTRL2 = ATLC_LC_LP_CTRL2__SLP_TM__WRITE(0);
    CMSDK_ATLC_NONSECURE->LC_LP_CTRL3 = ATLC_LC_LP_CTRL3__SLP__MASK;
    return 0;
}
...
static int ble_driver_early_init(void)
{
    // Required before the MCU is enabled, hence early
    pc_ctr_early_init();
    return 0;
}

SYS_INIT(ble_driver_early_init, EARLY, 0);
#endif

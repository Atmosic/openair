...
proc get_section_names {} {
    set section_names {".shstrtab" ".flash" ".rom" ".rram" ".sram" ".tcm"}
...
proc get_regions_from_luts { lut_list base_addr blk_size label max_blk} {
    set ret_list [list]
	...
	if { ($max_blk != 0) && ($remain_lut < 32)} {
	    set valid_lut $remain_lut;
	    if {$remain_lut <= 0} {
		break
	    }
	}
	...
	if {($lut == 0) || ($lut == 0xffffffff)} {
	    ...
	    incr counter $valid_lut
		 set remain_lut [expr {$remain_lut - $valid_lut}]
	} else {
	    for {set i 0} {$i < $valid_lut} {incr i} {
...
proc get_list_of_regions {} {
    ...
    set list_of_regions [concat $list_of_regions [get_regions_from_luts $rom_luts $::CMSDK_ROM_BASE $fls_blk_size ".rom" 0]]
    ...
    set list_of_regions [concat $list_of_regions [get_regions_from_luts $rram_luts $::CMSDK_RRAM_BASE $fls_blk_size ".rram" 0]]
    ...
    set tcm_override [mrw [::CMSDK_PSEQ${PSEQ_SX}::EMRAM_OVERRIDES]]
    set tcm_override_shift [::CMSDK_PSEQ${PSEQ_SX}::EMRAM_OVERRIDES::OVERRIDE_EMRAM_VDDCUT::SHIFT]
    ...
    set sram2_luts [list]
    set sram3_luts [list]
    set sram_size_per_blks [expr {32 * $sram_blk_size}]
    set num_sram_blks [expr {($sram_bank_size * $num_sram_banks_per_region) / $sram_size_per_blks}]
    ...
    set sram_luts_per_bank [expr {$sram_bank_size / $sram_size_per_blks}]
    set num_blk_per_bank [expr {$sram_bank_size / $sram_blk_size}]
	...
	if {$sram_luts_per_bank > 0} {
	    ...
	    set cur_bank_luts [lrange $cur_ram_luts $start_lut [expr {$start_lut +1}]]
	    set list_of_regions [concat $list_of_regions [get_regions_from_luts $cur_bank_luts $start_addr $sram_blk_size ".sram" 0]]
	} else {
	    set start_lut_bits [expr {(($start_addr - $::CMSDK_RAM_BASE) / $sram_blk_size) % ($num_blk_per_bank * $num_sram_banks_per_region)}]
	    set start_lut [expr {$start_lut_bits >> 5}]
	    set start_lut_bits [expr {$start_lut_bits & 0x1F}]
	    set cur_bank_luts [lindex $cur_ram_luts $start_lut]
	    set cur_bank_luts [expr {$cur_bank_luts >> $start_lut_bits}]
	    set list_of_regions [concat $list_of_regions [get_regions_from_luts $cur_bank_luts $start_addr $sram_blk_size ".sram" $num_blk_per_bank]]
	}
    }

    ####### ATLC TCM ########
    # Defines Secure or Non-secure address based on the ATLC PPC
    variable TCM_NS
    namespace eval ::SEC_PRIV_CTRL_SECURE::AHBNSPPCEXP1 {
	upvar TCM_NS TEMP_NS
	set ahbnsppcexp1_val [mrw [REG_ADDR]]
	set TEMP_NS [ATLC::READ $ahbnsppcexp1_val]
    }

    set tcm_bank_size 0x800

    # Check overrides, there are 2 bits but only bit 0 applies to TCM
    if {!(($tcm_override >> $tcm_override_shift) & 1)} {
	if {$TCM_NS} {
	    set start_addr $::CMSDK_TCM_BASE
	    set veneer_addr [expr { $::CMSDK_TCM_BASE | 0x10000000 }]
	} else {
	    set start_addr [expr { $::CMSDK_TCM_BASE | 0x10000000 }]
	    set veneer_addr $::CMSDK_TCM_BASE
	}
	lappend list_of_regions [list $start_addr $tcm_bank_size ".tcm" 0]
	lappend list_of_regions [list $veneer_addr $tcm_bank_size ".tcm" 1]
    }
    ...
    set list_of_regions [concat $list_of_regions [get_regions_from_luts $qspi_luts $::CMSDK_FLASH_BASE $fls_blk_size ".flash" 0]]
	...
	for {set i 0} {$i < (32 * $remainder)} {incr i} {
		...
		lappend list_of_regions [list $start_addr $region_size $label 0]
		lappend list_of_regions [list $veneer_addr $region_size $label 1]
		incr offset $region_size
		set counter 0
		set type $cur_val
	    }

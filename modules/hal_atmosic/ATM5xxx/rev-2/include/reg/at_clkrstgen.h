...
at_clkrstgen_get_bp(void)
{
	...
	uint32_t bp_freq = at_clkrstgen_pll_freq() /
	    CLKRSTGEN_CLKSYNC__DIV_BP_VAL__READ(
	...
	uint32_t bp_freq = at_clkrstgen_pll_freq() /
	    (2 * CLKRSTGEN_CLKSYNC__DIV_BP_VAL__READ(
	    CMSDK_CLKRSTGEN_NONSECURE->CLKSYNC));
	return (bp_freq);
    }
...
__CLKRSTGEN_SET_STATIC_INLINE void
at_clkrstgen_enable_pll(uint8_t sel, uint8_t div_bp, uint8_t div_qspi,
    bool set, bool commit)
{
    ...
    uint32_t clksync = CMSDK_CLKRSTGEN_NONSECURE->CLKSYNC;
    CLKRSTGEN_CLKSYNC__DIV_VAL__MODIFY(clksync, sel);
    CLKRSTGEN_CLKSYNC__DIV_BP_VAL__MODIFY(clksync, div_bp);
    CLKRSTGEN_CLKSYNC__DIV_QSPI_VAL__MODIFY(clksync, div_qspi);
    CMSDK_CLKRSTGEN_NONSECURE->CLKSYNC = clksync;
    CLKRSTGEN_CLKSYNC__CLK16_SRC__SET(clksync);
    CMSDK_CLKRSTGEN_NONSECURE->CLKSYNC = clksync;
}
...
{
    at_clkrstgen_enable_pll(sel, 1, 1, set, commit);
    if (!commit) {
	return;
    }
...
__CLKRSTGEN_SET_STATIC_INLINE void
at_clkrstgen_set_pll(uint8_t sel, uint8_t div_bp, uint8_t div_qspi,
    bool set, bool commit)
{
    at_clkrstgen_enable_pll(sel, div_bp, div_qspi, set, commit);
    if (!commit) {
	return;
    }
...
at_clkrstgen_asic_set_bp_hint(uint32_t freq, bool set, bool commit)
{
	...
	at_clkrstgen_set_pll(4, 4, 1, set, commit);
    } else if (freq <= 32000000) {
	at_clkrstgen_set_pll(4, 2, 1, set, commit);
    } else if (freq <= 48000000) {
	at_clkrstgen_set_pll(3, 1, 1, set, commit);
    } else /* 64MHz */ {
	at_clkrstgen_set_pll(4, 1, 1, set, commit);
    }
#else // __ICCARM__
    switch (freq) {
	...
	case 8000001 ... 16000000: at_clkrstgen_set_pll(4, 4, 2, set, commit); break;
	case 16000001 ... 32000000: at_clkrstgen_set_pll(4, 2, 2, set, commit); break;
	case 32000001 ... 48000000: at_clkrstgen_set_pll(3, 1, 1, set, commit); break;
	default: /* 64MHz */ at_clkrstgen_set_pll(4, 1, 1, set, commit); break;
    }
#endif // __ICCARM__
...
at_clkrstgen_fpga_set_bp(uint32_t index, uint32_t freq, bool set, bool commit)
{
	...
	at_clkrstgen_set_pll(4, 4, 2, set, commit);
    } else /* 32MHz */ {
	at_clkrstgen_set_pll(4, 2, 2, set, commit);
    }
#else
    switch (freq) {
	case 0 ... 500000: at_clkrstgen_set_slow(7, set, commit); break;

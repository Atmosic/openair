...
#if defined(CONFIG_SOC_FAMILY_ATM)
#define S_BOOT_ROM_ADDR \
    (DT_REG_ADDR(DT_MTD_FROM_FIXED_PARTITION(DT_NODELABEL(boot_rom))) + \
	DT_REG_ADDR(DT_NODELABEL(boot_rom)))
#define S_BOOT_ROM_SIZE DT_REG_SIZE(DT_NODELABEL(boot_rom))
#define ROM_VERSION_ADDR (S_BOOT_ROM_ADDR + S_BOOT_ROM_SIZE - sizeof(uint32_t))
#else
#define ROM_VERSION_ADDR (CMSDK_FLASH_BASE + ROM_SIZE - sizeof(uint32_t))
#endif

static void read_rom_version(void)
{
    rom_version = *(uint8_t *)(ROM_VERSION_ADDR);
}
#endif

...
// this option is available newer versions
#define SEC_DEV_LOCKOUT_ROM_P_LOCK_CFG_AVAIL
#endif
...
#ifndef CMSDK_SHUB_NONSECURE
#define SHUB_LOCKOUT_SUPPORT 0
#else
#define SHUB_LOCKOUT_SUPPORT 1
#endif
...
typedef enum {
    ...
    /// lockout UART2 Transmitter (output is disabled)
    SEC_DEV_LOCKOUT_UART2_TX,
#if SHUB_LOCKOUT_SUPPORT
    ...
    /// lockout RRAM remap configuration (locks default values)
    SEC_DEV_LOCKOUT_RRAM_REMAP,
    /// lockout QSPI remap configuration (locks default values)
    SEC_DEV_LOCKOUT_QSPI_REMAP,
#ifdef WRPRPINS_PROT_BITS_SET1__ROM_PATCH_LOCK_SET__MASK
    /// lockout ROM patch configuration from further changes
    SEC_DEV_LOCKOUT_ROM_P_LOCK_CFG,
#endif
    SEC_DEV_LOCKOUT_MAX,
...
/**
 * @brief Disable the RRAM remap interface
 *
 * @note: This function is called by sec_device_set_lockout
 * @note: Sets default values only
 *
 * @return true on success
 */
...
/**
 * @brief Disable the QSPI remap interface
 *
 * @note: This function is called by sec_device_set_lockout
 * @note: Sets default values only
 *
...
static inline uint32_t sec_dev_lock_get_mask(sec_device_lockout_type_t type)
{
	...
	case SEC_DEV_LOCKOUT_UART2_TX:
	    return WRPRPINS_PROT_BITS_SET1__UART2_LOCK_SET__MASK;
#if SHUB_LOCKOUT_SUPPORT
...
#ifdef WRPRPINS_PROT_BITS_SET1__ROM_PATCH_LOCK_SET__MASK
	case SEC_DEV_LOCKOUT_ROM_P_LOCK_CFG:
	    return WRPRPINS_PROT_BITS_SET1__ROM_PATCH_LOCK_SET__MASK;
#endif
	case SEC_DEV_LOCKOUT_RRAM_REMAP:
	case SEC_DEV_LOCKOUT_QSPI_REMAP:
...
{
    if (type == SEC_DEV_LOCKOUT_RRAM_REMAP) {
	return sec_device_disable_rram_remap();
    }

    if (type == SEC_DEV_LOCKOUT_QSPI_REMAP) {
	return sec_device_disable_qspi_remap();
    }

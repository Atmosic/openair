...
/**
 * @brief Invoked just before ATLC SLP bit is set
 */
...
/**
 * @brief Invoked at interrupt level from ATLC WU_SLP handler
 */
...
/**
 * @brief Allows multiple radio protocols to provide activity status to ATLC
 * for power management.
 * @return pc_ctr_sleep_e state.  Something other than PC_CTR_ATLC_TO_SLEEP
 * means that the caller should not attempt to put the ATLC to sleep.
 */
extern rep_vec__ret_uint32_t__t *rv_radio_sleep_state;
#define RV_RADIO_SLEEP_STATE_ADD(__f) \
    RV_ADD(__ret_uint32_t__, rv_radio_sleep_state, __f)
#define RV_RADIO_SLEEP_STATE_ADD_LAST(__f) \
    RV_ADD_LAST(__ret_uint32_t__, rv_radio_sleep_state, __f)

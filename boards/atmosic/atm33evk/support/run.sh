...
is_undef ATMWSTKLIB || {
    ...
    NS_EXTRAS="$NS_EXTRAS -DCONFIG_ATMWSTK_${ATMWSTKLIB}=y"
}

is_undef ATMWSTK || {
    NS_EXTRAS="$NS_EXTRAS -DCONFIG_ATMWSTK_${ATMWSTK}=y "
    DTS_EXTRAS="$DTS_EXTRAS;-DFIXED_ATMWSTK=${ATMWSTK};"
}
...
then
	...
	then
	    ${WBUILD[@]} -s $MCUBOOT -b $BOARD@mcuboot_bl -d build/$BOARD/$MCUBOOT -- $CMAKE_DEF_BOARD_ROOT -DCONFIG_DEBUG=n ${BL_EXTRAS} -DDTS_EXTRA_CPPFLAGS=${DTS_EXTRAS}${DTS_BL_EXTRAS}

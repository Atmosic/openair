...
then
	...
	then
	    ${WBUILD[@]} -s $MCUBOOT -b $BOARD@mcuboot -d build/$BOARD/$MCUBOOT -- $CMAKE_DEF_BOARD_ROOT -DCONFIG_BOOT_SIGNATURE_TYPE_ECDSA_P256=y -DCONFIG_BOOT_ECDSA_MICRO_ECC=y -DCONFIG_BOOT_SHA2_ATM=y -DCONFIG_DEBUG=n ${BL_EXTRAS} -DDTC_OVERLAY_FILE="${BOARD_ROOT:-$WEST_TOPDIR/openair}/boards/atmosic/atm33evk/${BOARD}_mcuboot_bl.overlay" -DDTS_EXTRA_CPPFLAGS=${DTS_EXTRAS}${DTS_BL_EXTRAS}

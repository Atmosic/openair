...
if not ZEPHYR_BASE:
    # set ZEPHYR_BASE.
    ZEPHYR_BASE = os.path.join(str(Path(__file__).resolve().parents[5]), "zephyr")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Atmosic Zephyr Generate " "Partition Info Map Tool"
    )
    ...
    gen_parser.add_argument(
        ...
        "--mcuboot_secondary_ext_flash",
        action="store_true",
        help="MCUBOOT_SECONDARY_EXT_FLASH enabled, " "default false",
    )
    gen_parser.add_argument(
        "-max_sector_size",
        "--max_sector_size",
        required=False,
        default=None,
        help="max sector size",
    )
    gen_parser.add_argument(
        "-rom_start", "--rom_start", required=False, default=None, help="rom start"
    )
    gen_parser.add_argument(
        "-rom_size", "--rom_size", required=False, default=None, help="rom size"
    )
    gen_parser.add_argument(
        "-user_data_offset",
        "--user_data_offset",
...
class AtmPartInfo:  # pylint: disable=too-many-instance-attributes,too-few-public-methods
    ...
    def __init__(self):  # pylint: disable=too-many-statements
        ...
        self.SLOT2_TRAILER_START = None
        self.SLOT2_TRAILER_OFFSET = None
        self.SLOT2_TRAILER_SIZE = None
    ...
    def merge_info(self):
        ...
        for key, value in self.input1_part_info.__dict__.items():
            if value is not None:
                if hasattr(self.input2_part_info, key) and getattr(
                    self.input2_part_info, key
                ):
...
class DevStreeParser:  # pylint: disable=too-many-instance-attributes
    ...
    def init(self):
        ...
        if self.merge_spe_nspe:
            self.part_info.MERGE_SPE_NSPE = 1
        if self.revision:
    ...
    def parsing_rram(self):  # pylint: disable=too-many-branches
        ...
        if not rram_controller:
            ...
            return
        ret, rram0_start, rram0_size = utils_get_node_property_reg(rram_controller)
        if ret == ST_ERROR:
            log_and_exit("Parsing rram_controller failed")
        self.debug_print(
            f"rram0_start = {hex(rram0_start)}, " f"rram0_size = {hex(rram0_size)}"
        )

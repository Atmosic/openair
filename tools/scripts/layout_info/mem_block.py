...
class MemBlock:
        ...
        for block in sorted_blocks:
            if prev_l_bound != -1:
                if prev_u_bound != block.l_bound:
                    reserved_blocks.append(ReservedBlock(
                        [prev_u_bound, block.l_bound]))
    ...
    def __str__(self):
        ...
        if len(self.sub_blocks) == 0:
            ...
            sub_delimit_str = f"{self.left_end_pp}" + \
...
class MemoryRegion(MemBlock):
    ...
    def __str__(self):
        ...
        if len(self.sub_blocks) == 0:
            ...
            sub_delimit_str = \
             f"{self.left_end_pp}{MemBlock.LIM_CH*MemBlock.BLOCK_WIDTH_PP}{self.right_end_pp}"
...
def auto_sort(blocks):
    ...
    return [b.promote_to_mem_region() for b in sorted(top_blocks)]

...
#include "arch.h"
#if !defined(CONFIG_PM) && defined(CONFIG_ATM_FAST_LP)
...
#define TEST_PATTERN_PSEUDO_RANDOM 0x42 /* Deterministic pseudo-random pattern */
...
static inline size_t get_max_test_data_size(void)
{
	...
	return (max_from_capacity < max_from_structure) ? max_from_capacity : max_from_structure;
}

/* Initialize test data sizes based on platform capacity */
static void init_test_data_sizes(void)
{
	...
	/* Adjust MEDIUM size - use default or 1/2 of available capacity */
	TEST_DATA_SIZE_MEDIUM = (max_data >= TEST_DATA_SIZE_MEDIUM_DEFAULT)
					? TEST_DATA_SIZE_MEDIUM_DEFAULT
					: (max_data / 2);

	/* Adjust LARGE size - use default or 3/4 of available capacity */
	TEST_DATA_SIZE_LARGE = (max_data >= TEST_DATA_SIZE_LARGE_DEFAULT)
	...
	if (TEST_DATA_SIZE_MEDIUM < 16) {
		TEST_DATA_SIZE_MEDIUM = 16;
	}
	if (TEST_DATA_SIZE_LARGE < 24) {
		TEST_DATA_SIZE_LARGE = 24;
	}
	...
	LOG_INF("  MEDIUM: %zu bytes (default: %d)", TEST_DATA_SIZE_MEDIUM,
		TEST_DATA_SIZE_MEDIUM_DEFAULT);
	LOG_INF("  LARGE: %zu bytes (default: %d)", TEST_DATA_SIZE_LARGE,
		TEST_DATA_SIZE_LARGE_DEFAULT);
	LOG_INF("  MAX: %zu bytes", max_data);
}
...
#elif defined(CONFIG_ATM_FAST_LP)
	LOG_INF("Using fast LP mode");
	atm_enter_fast_lp_mode(ATM_FAST_LP_MODE_HIB, atm_ms_to_lpc(HIB_TEST_DURATION_MS));
#else
	__ASSERT(0, "No HIB power cycling, should not be here!");
#endif
}
					   ...
					   const uint8_t *test_data)
{
	...
	combined_storage.test_state.checksum = calculate_checksum(test_data, data_size);
...
static bool load_combined_hibernation_data(hib_test_cycle_t *current_test, uint32_t *results,
					   size_t *data_size, uint8_t *pattern, uint8_t *test_data)
{
	/* Clear the combined storage */
	memset(&combined_storage, 0, sizeof(combined_storage));
	...
	LOG_DBG("Expected cycle_magic=0x%08x, test_magic=0x%08x", HIB_TEST_CYCLE_MAGIC,
		HIB_TEST_MAGIC);

	/* Validate cycle state */
	if (combined_storage.cycle_state.magic != HIB_TEST_CYCLE_MAGIC) {
		LOG_ERR("Invalid cycle state magic - retained memory data corrupted or not "
			"preserved\n");
		...
		for (int i = 0; i < 32; i += 8) {
			LOG_DBG("  %02d: %02x %02x %02x %02x %02x %02x %02x %02x", i, raw_data[i],
				raw_data[i + 1], raw_data[i + 2], raw_data[i + 3], raw_data[i + 4],
				raw_data[i + 5], raw_data[i + 6], raw_data[i + 7]);
		}
	...
	*results = combined_storage.cycle_state.test_results;
	...
	if (combined_storage.test_state.magic == HIB_TEST_MAGIC) {
		...
		*pattern = combined_storage.test_state.pattern;
		...
		LOG_DBG("Valid test state - data_size=%zu, pattern=0x%02x, checksum=0x%08x",
...
static bool test_hibernation_with_data(size_t data_size, uint8_t pattern, const char *test_name)
{
	LOG_INF("Verifying hibernation test: %s (size=%zu, pattern=0x%02x)", test_name, data_size,
		pattern);
	...
	uint32_t expected_checksum = calculate_checksum(test_data_buffer, state->data_size);
	uint32_t actual_checksum = calculate_checksum(restored_data_buffer, state->data_size);

	LOG_DBG("Checksum - Expected: 0x%08x, Actual: 0x%08x, Stored: 0x%08x", expected_checksum,
	...
	if (expected_checksum != actual_checksum) {
		...
		for (size_t i = 0; i < 8 && i < state->data_size; i++) {
			LOG_DBG("  [%zu] 0x%02x vs 0x%02x %s", i, test_data_buffer[i],
				...
				(test_data_buffer[i] == restored_data_buffer[i]) ? "OK" : "FAIL");
		}
		return false;
	}
...
static bool test_hib_hibernation_simple(void)
{
	...
	if (combined_storage.test_state.data_size != data_size) {
		LOG_ERR("Simple test data size mismatch: expected %zu, got %u\n", data_size,
			combined_storage.test_state.data_size);
		return false;
	}
	...
	if (IS_ENABLED(CONFIG_LOG)) {
		...
		for (int i = 0; i < 8; i++) {
			snprintf(hex_str + strlen(hex_str), sizeof(hex_str) - strlen(hex_str),
				 "0x%02x ", combined_storage.test_data[i]);
		}
...
static bool run_hibernation_test_verification(hib_test_cycle_t test_type)
{
	...
	case HIB_TEST_SMALL_DATA:
		return test_hibernation_with_data(TEST_DATA_SIZE_SMALL, TEST_PATTERN_0x55,
						  "small data");

	case HIB_TEST_MEDIUM_DATA:
		return test_hibernation_with_data(TEST_DATA_SIZE_MEDIUM, TEST_PATTERN_0xAA,
						  "medium data");

	case HIB_TEST_LARGE_DATA:
		return test_hibernation_with_data(TEST_DATA_SIZE_LARGE, TEST_PATTERN_PSEUDO_RANDOM,
						  "large data");

	case HIB_TEST_MAX_DATA:
		return test_hibernation_with_data(get_max_test_data_size(), TEST_PATTERN_0x55,
						  "max data");
...
static void do_test_size_validation(void)
{
	...
	LOG_INF("Compile-time max test data: %zu bytes", (size_t)TEST_DATA_SIZE_MAX);
	...
	size_t required_capacity = HIB_OVERHEAD_SIZE + get_max_test_data_size();
	LOG_INF("Required size: %zu bytes", required_capacity);

	/* Validate that our structure fits in the available capacity */
	zassert_true(required_capacity <= hib_capacity,
		     "Required capacity (%zu) exceeds HIB capacity (%zu)", required_capacity,
		     hib_capacity);

	/* Validate that we have reasonable capacity for testing */
	/* Platforms vary: expansion-based have 64 bytes, SHUB-based have up to 222 bytes */
	size_t min_capacity = HIB_OVERHEAD_SIZE + 8;
	zassert_true(hib_capacity >= min_capacity,
		     "Retained memory capacity (%zu) is too small (need at least %zu bytes)",
		     hib_capacity, min_capacity);
...
static void do_test_basic_save_restore(void)
{
	...
	size_t capacity = get_retained_mem_size();
	zassert_true(capacity > 0, "Failed to get retained memory size");

	/* Use platform's actual capacity, capped at 32 bytes for this test */
	...
	/* Save data */
	hib_save_data(test_data, test_size);

	/* Clear buffer */
	memset(restored_data, 0, sizeof(restored_data));

	/* Restore data */
	hib_restore_data(restored_data, test_size);
...
static void do_test_magic_values_preservation(void)
{
	...
	hib_restore_data((uint8_t *)&restored_cycle_state, sizeof(restored_cycle_state));
...
ZTEST(retained_mem, test_zz_hibernation_cycle)
{
...
#if !defined(CONFIG_PM) && !defined(CONFIG_ATM_FAST_LP)
	LOG_INF("=== PM Disabled, skip hibernate cycling test");
	ztest_test_skip();
	return;
#endif
	...
	if (is_boot_type(TYPE_HIB)) {
		...
		size_t data_size;
		uint8_t pattern;
		if (!load_combined_hibernation_data(&current_test, &test_results, &data_size,
						    &pattern, NULL)) {
			LOG_INF("WARNING: Could not load hibernation test data - retained memory "
				"may not be working");
			LOG_INF("This could indicate:");
			LOG_INF("1. Retained memory is not preserving data across hibernation");
			...
			zassert_true(false, "Hibernate cycling test FAILED");
			return;
		}

		LOG_INF("=== Hibernation Wakeup: Running %s Test ===", get_test_name(current_test));

		/* Re-initialize test data sizes (static vars reset after hibernation) */
		init_test_data_sizes();
		...
		if (next_test == HIB_TEST_COMPLETE) {
			...
			LOG_INF("\n=== Preparing Next Test: %s ===", get_test_name(next_test));
		...
		LOG_INF("2. Small Data (%zu bytes)", TEST_DATA_SIZE_SMALL);
		LOG_INF("3. Medium Data (%zu bytes)", TEST_DATA_SIZE_MEDIUM);
		LOG_INF("4. Large Data (%zu bytes, pseudo-random)", TEST_DATA_SIZE_LARGE);

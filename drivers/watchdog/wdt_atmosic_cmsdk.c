...
#define DT_DRV_COMPAT atmosic_cmsdk_watchdog

/**
 * @brief Atmosic CMSDK APB Watchdog driver with QSPI WAR integration.
 *
 * This driver extends the standard ARM CMSDK watchdog with Cairo QSPI
 * reset workaround. It ensures QSPI flash is ready before watchdog reset.
 */
...
LOG_MODULE_REGISTER(wdt_atmosic_cmsdk, CONFIG_WDOG_ATMOSIC_CMSDK_LOG_LEVEL);

/* External reference to QSPI WAR function from soc.c */
extern void qspi_wait_remote_status_clear(void);
...
static void wdog_cmsdk_apb_handler(void)
{
	...
	if (flags == WDT_FLAG_RESET_NONE) {
		 ...
		 * Execute QSPI workaround before reset
		 */
		LOG_DBG("Executing QSPI WAR in watchdog handler");
		/* Execute QSPI workaround to ensure flash is ready for ROM code */
		qspi_wait_remote_status_clear();
		LOG_DBG("QSPI WAR completed, proceeding to reset");
		/*

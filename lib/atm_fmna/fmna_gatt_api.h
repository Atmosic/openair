 ...
 * The service has a single combined control-point characteristic (nonowner_ctrl_uuid).
 */
...
ssize_t fmna_gatt_pairing_ctrl_write(struct bt_conn *conn, struct bt_gatt_attr const *attr,
				     void const *buf, uint16_t len, uint16_t offset, uint8_t flags);

ssize_t fmna_gatt_cfg_ctrl_write(struct bt_conn *conn, struct bt_gatt_attr const *attr,
				 void const *buf, uint16_t len, uint16_t offset, uint8_t flags);

ssize_t fmna_gatt_owner_info_ctrl_write(struct bt_conn *conn, struct bt_gatt_attr const *attr,
					void const *buf, uint16_t len, uint16_t offset,
					uint8_t flags);

ssize_t fmna_gatt_debug_ctrl_write(struct bt_conn *conn, struct bt_gatt_attr const *attr,
				   void const *buf, uint16_t len, uint16_t offset, uint8_t flags);

/* R3: Accessory Non-Owner Service write handler (combined control point). */
ssize_t fmna_gatt_nonowner_ctrl_write(struct bt_conn *conn, struct bt_gatt_attr const *attr,
				      void const *buf, uint16_t len, uint16_t offset,
				      uint8_t flags);

#ifdef CONFIG_USE_UARP
/* UARP write handler */
ssize_t fmna_gatt_uarp_data_write(struct bt_conn *conn, struct bt_gatt_attr const *attr,
				  void const *buf, uint16_t len, uint16_t offset, uint8_t flags);
#endif
 ...
 */
void fmna_gatt_register_services(struct bt_gatt_service_static const *fmn,
